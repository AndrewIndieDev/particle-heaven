...
namespace AndrewDowsett.Utility
{
    ...
    public class TwoKeyDictionary<T0, T1, T2>
    {
        List<T0> T0key = new List<T0>(); // the first reference - eg. string
        List<T1> T1key = new List<T1>(); // the second reference = eg. int
        List<T2> T2value = new List<T2>(); // the value both of these references will point to

        public int Count { get { return T2value.Count; } }

        public void Add(T0 T0key, T1 T1key, T2 value)
        {
            this.T0key.Add(T0key);
            this.T1key.Add(T1key);
            T2value.Add(value);
        }

        public void Remove(T0 key)
        {
            int index = T0key.FindIndex(x => x.Equals(key));
            if (index == -1) return;
            T0key.RemoveAt(index);
            T1key.RemoveAt(index);
            T2value.RemoveAt(index);
        }

        public void Remove(T1 key)
        {
            int index = T1key.FindIndex(x => x.Equals(key));
            if (index == -1) return;
            T0key.RemoveAt(index);
            T1key.RemoveAt(index);
            T2value.RemoveAt(index);
        ...
        public bool ReplaceKey(T0 key, T0 newKey)
        {
            int index = T0key.FindIndex(x => x.Equals(key));
        ...
        public bool ReplaceKey(T1 key, T1 newKey)
        {
            int index = T1key.FindIndex(x => x.Equals(key));
            if (index == -1) return false;
            T1key[index] = newKey;
        ...
        {
            return T0key.FindIndex(x => x.Equals(key)) != -1;
        }

        public bool ContainsKey(T1 key)
        {
            return T1key.FindIndex(x => x.Equals(key)) != -1;
        }

        public T2 Find(T0 key)
        {
            int index = T0key.FindIndex(x => x.Equals(key));
            if (index == -1) return default;
            return T2value[index];
        }
